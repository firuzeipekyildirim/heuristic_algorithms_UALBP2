...
\end{axis}
\end{tikzpicture}
\caption{Heatmap of the best cycle time for each mutation crossover combination.}
...
\begin{table}[H]
\centering
\caption{GA Performance Summary for Solved Benchmark Instances}\label{tab:ga_performance_summary}
...
\begin{itemize}
    \item The low variance of cycle times across independent runs indicates high reliability and repeatability of the proposed GA.
    \item For several benchmark instances, the algorithm consistently reaches the best solution in all runs, demonstrating strong convergence behavior ( small instances).
    \item For large and complex instances, the GA produces solutions very close to the optimum with limited dispersion, highlighting its robustness under tight precedence constraints.
    \item Runtime results show a clear dependency on problem size and complexity, while maintaining stable execution times across repeated runs.
    \item Overall, the GA effectively exploits the flexibility of U-shaped assembly lines and handles complex precedence structures without sacrificing solution quality.
\end{itemize}
